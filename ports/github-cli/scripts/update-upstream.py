#!/usr/bin/env python3
"""Update the pinned stable cli/cli release in config.env."""
...
def main() -> None:
    ...
    if tag_match is None:
        raise SystemExit("tag must look like v1.2.3")
    ...
    same_release = read_value(text, "GH_CLI_LAST_RELEASE") == args.tag
    text = replace_value(text, "GH_CLI_LAST_RELEASE", args.tag)
    text = replace_value(text, "GH_CLI_REVISION", args.revision)
